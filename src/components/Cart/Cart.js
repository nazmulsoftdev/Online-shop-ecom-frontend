...
const Cart = ({ setCartOpen, cartOpen }) => {
  ...
  return (
    <section
      className={`cart top-0 z-50 h-[100vh] bg-[#f7fdf6] lg:w-[30%] md:w-[40%] w-[80%] py-6 lg:px-8 px-4 ${
        cartOpen && "open"
      }`}
    >
      <label
        htmlFor="my-modal-3"
        className="btn btn-sm btn-circle btn-error text-white fixed right-5 top-2"
      ...
      {total > 0 ? (
        <h2 className="mt-4">
          Subtotal &emsp;{" "}
          <span className="text-3xl font-bold">BDT {total}</span>{" "}
        </h2>
      ) : (
      ...
      {cart?.length ? (
        <button
          ...
          className="btn bg-[#0a3d62] w-full mt-3 text-white normal-case"
        >
          Checkout &emsp;
          <lord-icon
            target="button"
